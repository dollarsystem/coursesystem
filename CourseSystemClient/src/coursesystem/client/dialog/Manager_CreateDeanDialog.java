 ...
 *
 */
public class Manager_CreateDeanDialog {
	private XMLContainer m_gui;
	private XMLContainer m_father;
	
	public Manager_CreateDeanDialog(XMLContainer p_gui) throws InstantiationException,IllegalAccessException,FileNotFoundException,ParserConfigurationException,SAXException,IOException
	{
		m_father=p_gui;
		m_gui=SmartGuis.create(new FileInputStream("res/dialog/Manager_CreateDeanDialog.xml"));
		assign();
	}
	...
	private void assign()
	{
		...
		t_save.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent t_evt)
			{
				...
				t_request.m_type=Manager_UserCreateRequest.DEAN;
				Dean s=new Dean();
				s.m_id=t_id.getText();
				...
				else
				{
					...
					.append(" dean_id='").append(s.m_id).append("' bgcolor='#cbfef1'/>");
					try 
					{
