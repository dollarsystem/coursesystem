...
public class CreateStudentDialog {
	...
	public CreateStudentDialog(XMLContainer p_gui) throws InstantiationException,IllegalAccessException,FileNotFoundException,ParserConfigurationException,SAXException,IOException
	{
		m_father=p_gui;
		m_gui=SmartGuis.create(new FileInputStream("res/dialog/CreateStudentDialog.xml"));
	...
	private void assign()
	{
		...
		t_save.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent t_evt)
			{
				...
				t_request.m_type=CreateUserRequest.STUDENT;
				Student s=new Student();
				s.m_id=t_id.getText();
				s.m_name=t_name.getText();
				s.m_sex=t_sex.getText();
				s.m_faculty_id=AllStudentInitHandler.getSelectedFacultyId(m_gui);
				s.m_grade=AllStudentInitHandler.getSelectedGrade(m_gui);
				...
				else
				{
					...
					.append(" student_id='").append(s.m_id).append("' />");
					try 
					{
					...
					SmartGuis.find(m_father, "last_opt").set("text", " 新建学生 "+s.m_id);
				}
			}
		});
	}
