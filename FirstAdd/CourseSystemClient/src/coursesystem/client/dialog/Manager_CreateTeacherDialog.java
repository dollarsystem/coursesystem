 ...
 *
 */
public class Manager_CreateTeacherDialog {
	private XMLContainer m_gui;
	private XMLContainer m_father;
	//private String m_student_id;
	
	public Manager_CreateTeacherDialog(XMLContainer p_gui) throws InstantiationException,IllegalAccessException,FileNotFoundException,ParserConfigurationException,SAXException,IOException
	{
		m_father=p_gui;
		m_gui=SmartGuis.create(new FileInputStream("res/dialog/Manager_CreateTeacherDialog.xml"));
		assign();
	}
	...
	private void assign()
	{
		...
		t_save.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent t_evt)
			{
				...
				t_request.m_type=Manager_UserCreateRequest.TEACHER;
				...
				else
				{
					...
					.append(" teacher_id='").append(s.m_id).append("' bgcolor='#cbfef1'/>");
					try 
					{
