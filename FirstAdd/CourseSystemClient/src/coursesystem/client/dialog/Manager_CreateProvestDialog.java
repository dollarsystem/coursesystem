 ...
 *
 */
public class Manager_CreateProvestDialog {
	private XMLContainer m_gui;
	private XMLContainer m_father;
	
	public Manager_CreateProvestDialog(XMLContainer p_gui) throws InstantiationException,IllegalAccessException,FileNotFoundException,ParserConfigurationException,SAXException,IOException
	{
		m_father=p_gui;
		m_gui=SmartGuis.create(new FileInputStream("res/dialog/Manager_CreateProvestDialog.xml"));
		assign();
	}
	...
	private void assign()
	{
		...
		t_save.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent t_evt)
			{
				...
				t_request.m_type=Manager_UserCreateRequest.PROVEST;
				...
				else
				{
					...
					.append(" provest_id='").append(s.m_id).append("' bgcolor='#cbfef1'/>");
					try 
					{
